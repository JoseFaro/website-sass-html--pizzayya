CNVS.OnePage = function() {
	...
	var _scroll = function(section, settings, timeout) {
		setTimeout( function() {
			...
			// re-scroll before reaching final position to fix lazy load images delay
			setTimeout(function() {
				var fixedSectionOffset = __core.offset(section).top;
				__core.scrollTo((fixedSectionOffset - Number(settings.offset)), 100, settings.easing);
			}, settings.speed);
